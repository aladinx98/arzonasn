...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    const newUser = new User({ username, email, referralAddress, referredBy, password: hashedPassword, otp });
    await newUser.save();
...
router.post('/connectWallet', authUser, async (req, res) => {
  ...
  try {
    ...
    if (user.walletAddress) {
      return res.status(400).json({ message: 'Wallet address has already been connected' });
    }
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      if (match) {
        const token = jwt.sign({ userId: user._id }, JWT_ACCESS_SECRET, { expiresIn: '1h' });
